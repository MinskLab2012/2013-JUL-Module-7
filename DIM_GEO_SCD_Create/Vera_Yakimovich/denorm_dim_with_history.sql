 WITH piv AS (SELECT *
      ...
      , TO_CHAR(TRUNC( (SELECT max(action_dt) FROM T_GEO_OBJECT_ACTIONS tac WHERE tac.geo_id = piv.roo) )  , 'DD-MON-YY') as VALID_FROM
    ...
    FROM piv
         ...
         LEFT JOIN T_GEO_OBJECT_ACTIONS ta
...
UNION ALL
       ...
       , NVL ( lcp.part_id, -99 ) AS part_id
       , NVL ( lcp.part_desc, 'n.d.' ) AS part_desc
       , TO_CHAR(TRUNC(action_dt), 'DD-MON-YY')
                    as VALID_FROM
       , TO_CHAR(TRUNC(action_dt), 'DD-MON-YY') AS VALID_TO
       , 'N' AS IS_ACTUAL
    FROM piv         
         INNER JOIN T_GEO_OBJECT_ACTIONS ta

...
stmt2 varchar2 (500);
begin
delete from fct_trans_monthly_2008
where (event_dt, ship_id, arr_time_dt, arr_port_id,  dep_time_dt, dep_port_id, company_id, product_id, insurance_id, pct_goods, income, amount_tot) in (select event_dt, ship_id, arr_time_dt, arr_port_id, dep_time_dt, dep_port_id,
company_id, product_id, insurance_id, pct_goods, income, amount_tot from fct_trans_monthly_2008 minus select * from (select trunc(tr.tran_id,'MM'), tr.ship_id, tr.ar_time, tr.ar_port_id, tr.dep_time, tr.dep_port_id, tr.customer_id,tr.prod_id,  tr.insurance_id, avg(round(tr.dep_goods/tr.ar_goods*100,2)),sum( t.income_coef*dep_goods), count(tr.dep_goods)
from st.t_trans tr join st.t_products t
on tr.prod_id = t.prod_id
where trunc(tr.tran_id,'YYYY') = year
group by trunc(tr.tran_id,'MM'), tr.ship_id, tr.ar_time, tr.ar_port_id, tr.dep_time, tr.dep_port_id, tr.customer_id,tr.prod_id,  tr.insurance_id));
commit;

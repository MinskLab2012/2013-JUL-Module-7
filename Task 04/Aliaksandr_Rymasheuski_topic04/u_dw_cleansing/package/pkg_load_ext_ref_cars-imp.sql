/* Formatted on 8/5/2013 9:39:25 AM (QP5 v5.139.911.3011) */
...
AS
   ...
   BEGIN
      --Delete old values
      DELETE FROM u_dw.brands br
            WHERE br.brand_code NOT IN (SELECT DISTINCT brand_id
                                          FROM u_sa_data.contracts);

      --Merge Source data
      MERGE INTO u_dw.brands br
           USING (  SELECT DISTINCT brand_id
                                  , brand
                      FROM u_sa_data.contracts
                  ORDER BY brand) cls
      ...
      IS
         SELECT DISTINCT cn.model_id
           ...
           FROM u_sa_data.contracts cn
                JOIN u_dw.brands br
                   ON ( cn.brand_id = br.brand_code )
                LEFT JOIN u_dw.models omod
                   ON ( cn.model_id = omod.model_code );
   BEGIN
      --DELETE old   VALUES
      DELETE FROM u_dw.models ml
            WHERE ml.model_code NOT IN (SELECT DISTINCT model_id
                                          FROM u_sa_data.contracts);
   ...
   AS
      ...
      IS
         SELECT DISTINCT cn.car_id
                       ...
                       , cn.cost
                       , cn.status
           FROM u_sa_data.contracts cn JOIN u_dw.models md ON ( cn.model_id = md.model_code )
          WHERE ( cn.car_id, cn.cat_stand_num ) NOT IN (SELECT DISTINCT car_code
                                                                      , car_stand_number
                                                          FROM u_dw.cars);

      CURSOR c2 --dataset with the same id, we should update if dillerent desc
      IS

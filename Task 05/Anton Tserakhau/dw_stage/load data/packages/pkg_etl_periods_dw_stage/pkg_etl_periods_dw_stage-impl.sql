...
AS
   ...
   BEGIN
      DECLARE
	   ...
	   query_insert   VARCHAR2 ( 1000 );
	   query_update   VARCHAR2 ( 1000 );
	BEGIN
	   ...
	   BULK COLLECT INTO record_period_type_id_stage
	                   , record_period_type_name_source
	                   , record_period_type_name_stage
	                   , record_period_type_desc_source
	                   , record_period_type_desc_stage;
	
	   CLOSE cur_type_period;
	
	   query_insert := 'INSERT INTO t_type_periods ( period_type_id
	                                    , period_type_name
	                                    , period_type_desc
	                                    , insert_dt)
	              VALUES ( :a
	   ...
	   query_update := 'UPDATE t_type_periods
	            SET period_type_desc = :x
	              , update_dt    = :y
	          WHERE t_type_periods.period_type_id = :z';
	   ...
	   FOR i IN record_period_type_id_stage.FIRST .. record_period_type_id_stage.LAST LOOP
	      IF ( record_period_type_id_stage ( i ) IS NULL ) THEN
	         EXECUTE IMMEDIATE query_insert
	            USING seq_t_type_periods.NEXTVAL
	      ...
	      ELSIF ( record_period_type_desc_source ( i ) <> record_period_type_desc_stage ( i ) ) THEN
	         EXECUTE IMMEDIATE query_update
	            USING record_period_type_desc_source ( i )
	                , SYSDATE
	                , record_period_type_id_stage ( i );
   ...
   BEGIN
	...
	BEGIN
	   ...
	   query_insert := 'INSERT INTO t_periods ( period_id
	                                   , period_code
	                                   , period_desc
	                                   , period_type_id
	                                   , start_dt
	   ...
	   query_update := 'UPDATE t_periods
	                SET period_desc  = :x
	                  , period_type_id = :y
	                  , start_dt     = :z
	                  , end_dt       = :w
	                  , update_dt    = :v
	              WHERE t_periods.period_id = :u';
	   ...
	   FOR i IN record_period_id_stage.FIRST .. record_period_id_stage.LAST LOOP
	      ...
	      ELSIF ( record_period_desc_source ( i ) <> record_period_desc_stage ( i )
	         ...
	         EXECUTE IMMEDIATE query_update
	            USING record_period_desc_source ( i )
	                , record_period_type_id_source ( i )
	                , record_start_dt_source ( i )
	                , record_end_dt_source ( i )
	                , SYSDATE
	                , record_period_id_stage ( i );

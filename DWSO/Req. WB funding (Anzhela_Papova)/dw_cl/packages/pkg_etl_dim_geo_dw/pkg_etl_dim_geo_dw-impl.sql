...
AS
   ...
   BEGIN
      ...
      MERGE INTO dw.vl_countries trg
   ...
   BEGIN
      ...
      EXECUTE IMMEDIATE 'DELETE FROM U_DW_EXT_REFERENCES.CLS_GEO_STRUCTURE_ISO3166';

      --Extract data
      INSERT INTO U_DW_EXT_REFERENCES.cls_geo_structure_iso3166 ( child_code
                                            , parent_code
                                            , structure_desc
                                            , structure_level )
         SELECT child_code
              ...
              , structure_desc
              , structure_level
           FROM U_DW_EXT_REFERENCES.t_ext_geo_structure_iso3166;
   ...
   BEGIN
      --truncate cleansing tables
      EXECUTE IMMEDIATE 'DELETE FROM U_DW_EXT_REFERENCES.CLS_CNTR2STRUCTURE_ISO3166';

      --Extract data
      INSERT INTO U_DW_EXT_REFERENCES.cls_cntr2structure_iso3166 ( country_id
                                             , county_desc
                                             , structure_code
                                             , structure_desc )
         SELECT country_id
              ...
              , structure_code
              , structure_desc
           FROM U_DW_EXT_REFERENCES.t_ext_cntr2structure_iso3166;
   ...
   BEGIN
      --Delete old values
      DELETE FROM dw.w_geo_systems trg
            WHERE trg.geo_system_id NOT IN (SELECT DISTINCT child_code
                                              FROM U_DW_EXT_REFERENCES.cls_geo_structure_iso3166
                                             WHERE UPPER ( structure_level ) = 'WORLD');

      --Merge Source data
      MERGE INTO dw.w_geo_systems trg
      ...
      --Merge Source Localizable Data
      MERGE INTO dw.vl_geo_systems trg
           USING (SELECT geo_id
                       , geo_system_id
                       ...
                       , 1 AS localization_id
                    FROM dw.w_geo_systems src
                       , U_DW_EXT_REFERENCES.cls_geo_structure_iso3166 cls
   ...
   BEGIN
      ...
      --Merge Source Localizable Data
      MERGE INTO dw.vl_geo_parts trg
           USING (SELECT geo_id
                       , src.part_id
                       , NULL part_code
                       , structure_desc AS part_desc
                       , 1 AS localization_id
                    FROM dw.w_geo_parts src
                       , U_DW_EXT_REFERENCES.cls_geo_structure_iso3166 cls
   ...
   BEGIN
      ...
      --Merge Source Localizable Data
      MERGE INTO dw.vl_geo_regions trg
           USING (SELECT geo_id
                       , src.region_id
                       , NULL region_code
                       , structure_desc AS region_desc
                       , 1 AS localization_id
                    FROM dw.w_geo_regions src
                       , u_dw_ext_references.cls_geo_structure_iso3166 cls
   ...
   BEGIN
      --truncate cleansing tables
      EXECUTE IMMEDIATE 'DELETE FROM U_DW_EXT_REFERENCES.CLS_CNTR_GROUPING_ISO3166';

      --Extract data
      INSERT INTO u_dw_ext_references.cls_cntr_grouping_iso3166 ( child_code
                                            , parent_code
                                            , group_desc
                                            , group_level )
         SELECT child_code
              ...
              , group_desc
              , group_level
           FROM u_dw_ext_references.t_ext_cntr_grouping_iso3166;
   ...
   BEGIN
      --truncate cleansing tables
      EXECUTE IMMEDIATE 'DELETE FROM U_DW_EXT_REFERENCES.cls_cntr2grouping_iso3166';

      --Extract data
      INSERT INTO u_dw_ext_references.cls_cntr2grouping_iso3166 ( country_id
                                            , county_desc
                                            , group_code
                                            , group_desc )
         SELECT country_id
              ...
              , group_code
              , group_desc
           FROM u_dw_ext_references.t_ext_cntr2grouping_iso3166;
   ...
   BEGIN
      --Delete old values
      DELETE FROM dw.w_cntr_group_systems trg
            WHERE trg.grp_system_id NOT IN (SELECT DISTINCT child_code
                                              FROM u_dw_ext_references.cls_cntr_grouping_iso3166
                                             WHERE UPPER ( group_level ) = 'ALL');

      --Merge Source data
      MERGE INTO dw.w_cntr_group_systems trg
           USING (SELECT DISTINCT child_code
                    FROM u_dw_ext_references.cls_cntr_grouping_iso3166
      ...
      --Merge Source Localizable Data
      MERGE INTO dw.vl_cntr_group_systems trg
           USING (SELECT geo_id
                       , grp_system_id
                       , CASE WHEN ( grp_system_id = 1 ) THEN 'MAIN' ELSE NULL END grp_system_code
                       , cls.group_desc grp_system_desc
                       , 1 AS localization_id
                    FROM dw.w_cntr_group_systems src
                       , u_dw_ext_references.cls_cntr_grouping_iso3166 cls
   ...
   BEGIN
      ...
      --Merge Source Localizable Data
      MERGE INTO dw.vl_cntr_groups trg
           USING (SELECT geo_id
                       , src.GROUP_ID
                       , NULL group_code
                       , group_desc AS group_desc
                       , 1 AS localization_id
                    FROM dw.w_cntr_groups src
                       , u_dw_ext_references.cls_cntr_grouping_iso3166 cls
   ...
   BEGIN
      ...
      --Merge Source Localizable Data
      MERGE INTO dw.vl_cntr_sub_groups trg
           USING (SELECT geo_id
                       , src.sub_group_id
                       , NULL sub_group_code
                       , group_desc AS sub_group_desc
                       , 1 AS localization_id
                    FROM dw.w_cntr_sub_groups src
                       , u_dw_ext_references.cls_cntr_grouping_iso3166 cls
   ...
   BEGIN
      --Merge Source data
      MERGE INTO dw.w_geo_object_links trg
           USING (SELECT p_obj.geo_id parent_geo_id
                    ...
                    FROM (SELECT child_code
                       ...
                       , dw.w_geo_objects p_obj
                       , dw.w_geo_objects c_obj
   ...
   BEGIN
      ...
      MERGE INTO dw.w_geo_object_links trg
           USING (SELECT reg.geo_id AS parent_geo_id
                       ...
                       , cls.structure_desc
                       , 3 AS link_type_id
                    FROM u_dw_ext_references.cls_cntr2structure_iso3166 cls
                       , dw.w_countries cntr
                       , dw.w_geo_regions reg
           ...
           USING (SELECT p_obj.geo_id parent_geo_id
                    ...
                    FROM (SELECT child_code
                               ...
                               , CASE
                                 ...
                                 END
                                    AS type_id
                            FROM u_dw_ext_references.cls_cntr_grouping_iso3166) cls
                       , dw.w_geo_objects p_obj
                       , dw.w_geo_objects c_obj
                   WHERE cls.parent_code IS NOT NULL
                     AND p_obj.geo_code_id = cls.parent_code
   ...
   BEGIN
      ...
      MERGE INTO dw.w_geo_object_links trg
           USING (SELECT spb.geo_id AS parent_geo_id
                       ...
                       , 6 AS link_type_id
                       , cls.county_desc
                    FROM u_dw_ext_references.cls_cntr2grouping_iso3166 cls
                       , dw.w_cntr_sub_groups spb
                       , dw.w_countries cntr

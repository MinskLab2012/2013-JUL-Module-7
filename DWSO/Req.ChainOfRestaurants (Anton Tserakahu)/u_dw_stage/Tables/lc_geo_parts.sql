...
--
drop table u_dw_stage.lc_geo_parts cascade constraints;

--==============================================================
-- Table: lc_geo_parts                                          
--==============================================================
create table u_dw_stage.lc_geo_parts 
(
   geo_id               NUMBER(22,0)         not null,
   part_id              NUMBER(22,0)         not null,
   part_code            VARCHAR2(30 CHAR),
   part_desc            VARCHAR2(200 CHAR)   not null,
   constraint PK_LC_GEO_PARTS primary key (geo_id)
         using index tablespace ts_dw_stage_idx_01
)
tablespace ts_dw_stage_data_01;

comment on table u_dw_stage.lc_geo_parts is
'Localization table: T_GEO_SYSTEMS';

comment on column u_dw_stage.lc_geo_parts.geo_id is
'Unique ID for All Geography objects';

comment on column u_dw_stage.lc_geo_parts.part_id is
'ID Code of Part of World';

comment on column u_dw_stage.lc_geo_parts.part_code is
'Code of Part of World';

comment on column u_dw_stage.lc_geo_parts.part_desc is
'Description of Part of World';


alter table u_dw_stage.lc_geo_parts
...
alter table u_dw_stage.lc_geo_parts
   add constraint FK_LC_CONTINENTS foreign key (geo_id)
      references u_dw_stage.t_geo_parts (geo_id)
      on delete cascade;
